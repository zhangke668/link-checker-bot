import { createClient } from "@supabase/supabase-js";
  ...
  // 配置
  const MAX_LINKS_PER_RUN = 5000;  // 每次最多检测数量
  const CONCURRENCY = 20;  // 并发数
  const LINK_CHECK_TIMEOUT = 10000;  // 单个链接超时时间（毫秒）
  ...
  async function fetchWithTimeout(
    url: string,
    options: RequestInit,
    timeout: number = LINK_CHECK_TIMEOUT
  ): Promise<Response> {
   ...
   * 检查网盘链接是否有效
   */
  async function checkLinkStatus(url: string): Promise<{ valid: 
  boolean | null; reason?: string }> {
    try {
      // 夸克网盘
      if (url.includes("quark.cn")) {
        const match = url.match(/\/s\/([a-zA-Z0-9]+)/);
        if (!match) return { valid: false, reason:
  "链接格式无效" };
        ...
        const tokenUrl =
  `https://drive-h.quark.cn/1/clouddrive/share/sharepage/token?p
  r=ucpro&fr=pc&uc_param_str=&__dt=2000&__t=${Date.now()}`;     
        ...
        if (tokenData.code === 41003) return { valid: false,    
  reason: "分享已过期" };
        if (tokenData.code === 41006) return { valid: false,    
  reason: "分享已取消" };
        if (tokenData.code !== 0) return { valid: false, reason:
   tokenData.message || "链接无效" };

        const stoken = tokenData.data?.stoken;
        if (stoken) {
          const detailUrl =
  `https://drive-h.quark.cn/1/clouddrive/share/sharepage/detail?
  pr=ucpro&fr=pc&pwd_id=${pwd_id}&stoken=${encodeURIComponent(st
  oken)}&pdir_fid=0&force=0&_page=1&_size=50`;
          const detailRes = await fetchWithTimeout(detailUrl,   
  {});
          const detailData = await detailRes.json();
          if (detailData.code === 0 &&
  (detailData.metadata?._total || 0) === 0) {
  ...
  url.includes("yun.baidu.com")) {
        const res = await fetchWithTimeout(url, {
          headers: {
            "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS   
  13_2_3 like Mac OS X) AppleWebKit/605.1.15",
          },
          redirect: "manual",
        });
  ...
  async function checkBatch(
    links: Array<{ id: string; url: string; table: string;      
  urlField: string; statusField: string; checkedField: string   
  }>,
    startIndex: number,
    totalCount: number
  ) {
  ...
  1}/${totalCount}]`;
        try {
          ...
          const status = result.valid === true ? "valid" :      
  result.valid === false ? "expired" : "unchecked";
          ...
          await supabase
            ...
            .update({
              ...
              [link.checkedField]: new Date().toISOString(),    
            })
            .eq("id", link.id);

          const icon = status === "valid" ? "✓" : status ===    
  "expired" ? "✗" : "?";
          console.log(`${progress} ${icon} [${link.table}]      
  ${link.url.slice(0, 60)}... - ${result.reason || status}`);   

          return { status, error: false };
        } catch (e) {
          console.log(`${progress} ! [${link.table}]
  ${link.url.slice(0, 60)}... - 检测出错`);
  ...
  async function main() {
    ...
    for (const table of TABLES) {
      ...
      // 获取链接（按 last_checked 排序，最久没检测的优先）     
      const { data, error } = await supabase
        .from(table.name)
        .select(`id, ${table.urlField}, ${table.checkedField}`) 
        .order(table.checkedField, { ascending: true,
  nullsFirst: true })
        .limit(MAX_LINKS_PER_RUN);

      if (error) {
        console.error(`获取 ${table.name} 失败:`, error);       
        continue;
      }
  ...
  CONCURRENCY) * 2 / 60)} 分钟\n`);
    ...
    for (let i = 0; i < toCheck.length; i += CONCURRENCY) {     
      ...
      const results = await checkBatch(batch, i,
  toCheck.length);
    ...
    console.log(`有效: ${valid} | 失效: ${expired} | 未知:      
  ${unknown} | 错误: ${errors}`);
    console.log("========================================");    
  }
